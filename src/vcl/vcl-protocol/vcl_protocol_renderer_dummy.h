...
static inline void vcl_decode_clEnqueueMigrateMemObjects_args_temp(struct vcl_cs_decoder *dec, struct vcl_command_clEnqueueMigrateMemObjects *args)
{
    ...
    if (vcl_decode_simple_pointer(dec)) {
        args->mem_objects = vcl_cs_decoder_alloc_temp(dec, sizeof(*args->mem_objects));
        if (!args->mem_objects) return;
        vcl_decode_cl_mem_lookup(dec, (cl_mem *)args->mem_objects);
    } else {
...
static inline void vcl_replace_clEnqueueMigrateMemObjects_args_handle(struct vcl_command_clEnqueueMigrateMemObjects *args)
{
    ...
    if (args->mem_objects)
        vcl_replace_cl_mem_handle((cl_mem *)args->mem_objects);
    /* skip args->flags */
    /* skip args->num_events_in_wait_list */
    if (args->event_wait_list)
        vcl_replace_cl_event_handle((cl_event *)args->event_wait_list);
...
static inline void vcl_decode_clEnqueueWaitForEvents_args_temp(struct vcl_cs_decoder *dec, struct vcl_command_clEnqueueWaitForEvents *args)
{
    ...
    if (vcl_decode_simple_pointer(dec)) {
        args->event_list = vcl_cs_decoder_alloc_temp(dec, sizeof(*args->event_list));
        if (!args->event_list) return;
        vcl_decode_cl_event_lookup(dec, (cl_event *)args->event_list);
    } else {
...
static inline void vcl_replace_clEnqueueWaitForEvents_args_handle(struct vcl_command_clEnqueueWaitForEvents *args)
{
    ...
    if (args->event_list)
        vcl_replace_cl_event_handle((cl_event *)args->event_list);
}
#endif // CL_API_SUFFIX__VERSION_1_1_DEPRECATED
